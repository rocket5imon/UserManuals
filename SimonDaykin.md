...
---
## Introduction
This document is a *User Manual* for the system called **Simon Daykin** of the homosapien architecture.  The purpose of the document is to give interfacing systems and users an understanding of how to get the best out of the system, including a plethora of user information such as:
...
Where textual communication is used, particularly via fast transmission interfaces such as Instant Messaging, Group Chat & Email, the system attempts to always output concise and clear streams or files.  The systems processor includes the dyslexia instruction set, and thus occasionally this informal written communication can appear mysterious, ambiguous or generate error conditions at the receiving systems.  If this is the case, it is always best to validate with the source system.  No error codes are generated by seeking clarification.
